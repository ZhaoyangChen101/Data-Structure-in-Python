...
class Plant:
    ...
    def __hash__(self):
        ...
        """
        # raise NotImplementedError("Fix me!")
        return hash(self.name) * (hash(self.height) // 10)
...
class PlantShop():
    ...
    def fillStock(self, plant, quantity=1, price=1):
        ...
        # raise NotImplementedError("Fix me!")
        plantAttribute = self.stock.get(plant, False)
        if plantAttribute:
            self.stock[plant] = (plantAttribute[0] + quantity, plantAttribute[1])
        else:
    ...
    def sell(self, plant, pieces=1):
        ...
        # raise NotImplementedError("Fix me!")
        # if plant in self.available():
        #     price = self.stock[plant][1]
        #     quantity = self.stock[plant][0]
        #
        #     if pieces <= quantity:
        #         self.stock[plant] = (quantity - pieces, price)
        #         self.assets += price * pieces
        #
        #     else:
        #         # sell all plants in stock
        #         self.stock[plant] = (0, price)
        #         self.assets += price * quantity

        price = self.stock[plant][1]
        quantity = self.stock[plant][0]
        price_with_discount = price * (1 - self.discount / 100.0)

        if pieces <= quantity:
            self.stock[plant] = (quantity - pieces, price)
            if plant in self.onSale:
                self.assets += price_with_discount * pieces
            else:
                self.assets += price * pieces
        else:
            # sell all plants in stock
            self.stock[plant] = (0, price)
            if plant in self.onSale:
                self.assets += price_with_discount * quantity
            else:
                self.assets += price * quantity
    ...
    def putOnSale(self, plant):
        ...
        # update plant's price in stock
        # quantity = self.stock[plant][0]
        # price = self.stock[plant][1]
        # price_with_discount = price * (1 - self.discount / 100.0)
        # self.stock[plant] = (quantity, price_with_discount)
    ...
    def removeFromSale(self, plant):
        ...
        if plant in self.onSale:
            ...
            # update plant's price in stock
            # quantity = self.stock[plant][0]
            # price_with_discount = self.stock[plant][1]
            # original_price = price_with_discount / (1 - self.discount / 100.0)
            # self.stock[plant] = (quantity, original_price)
        else:
            print(plant, "is not on sale.")
    ...
    def available(self):
        ...
        for key, value in self.stock.items():
            if value[0] > 0:
                plants_available.append(key)
        return plants_available
