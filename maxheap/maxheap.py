...
class MaxHeap:
    ...
    def _higher_priority(self, i, j):
        ...
        #raise NotImplementedError("MaxHeap._higher_priority not yet implemented.")
        return self.array[i][0] > self.array[j][0]
    ...
    def insert(self, pair):
        ...
        self.size += 1
        index_pair = self.size - 1
        self._heapify_up(index_pair)

    def top(self):
        """Return the object at the top of the heap or None if heap is empty. """
        if self.is_empty():
            return None
        #raise NotImplementedError("MaxHeap.top not yet implemented.")
    ...
    def _heapify_down(self, i):
        ...
        while 0 <= i < self.size and not self._is_leaf(i):
            #raise NotImplementedError("MaxHeap._heapify_down not yet implemented.")
            i_left = self._left_child(i)
            i_left_priority = self.array[i_left][0]
            i_right = self._right_child(i)
            i_right_priority = self.array[i_right][0]
            max_index = i
            max_priority = self.array[max_index][0]
            if i_left_priority > max_priority:
                max_index = i_left
                max_priority = i_left_priority
            if i_right_priority > max_priority:
                max_index = i_right
                #max_priority = i_right_priority
    ...
    def pop(self):
        ...
        #raise NotImplementedError("MaxHeap.pop not yet implemented.")
        else:
            task = self.top()
