...
class AccountTree:
    ...
    def insert(self, new_key, new_value, parent_key=None, left_sibling_key=None):
        ...
        if left_sibling is None:
            raise KeyError(
                "Node with key {0} is not a child of a node with key {1}.".format(left_sibling_key, parent_key))
    ...
    def _visit_preorder(self, node):
        ...
        def preorder(temp_list, root):
            if root is None:
                return
            temp_list.append(root)
            preorder(temp_list, root.left_child)
            if root.left_child is None:
                return
            right = root.left_child.right_sibling
            while right is not None:
                preorder(temp_list, right)
    ...
    def _visit_postorder(self, node):
        ...
        def postorder(temp_list, root):
            if root is None:
                return
            # if root.left_child is None:
            #     temp_list.append(root)
            #     return
            postorder(temp_list, root.left_child)
            if root.left_child is not None:
                right = root.left_child.right_sibling
                while right is not None:
                    postorder(temp_list, right)
                    right = right.right_sibling
            temp_list.append(root)

        postorder_list = []
        postorder(postorder_list, node)
